...
describe('POST /api/auth/register', () => {
  ...
  it('400 when schema fails (username too short / weak password)', async () => {
    ...
    expect(json.fieldErrors.username.length).toBeGreaterThan(0);
    expect(Array.isArray(json.fieldErrors.password)).toBe(true);
    expect(json.fieldErrors.password.length).toBeGreaterThan(0);
  });
  ...
  it('400 when username already exists', async () => {
    ...
    expect(json.fieldErrors).toBeTruthy();
    expect(Array.isArray(json.fieldErrors.username)).toBe(true);
    const msg = String(json.fieldErrors.username[0] || '').toLowerCase();
